<template>
  <div class="bg-surface border rounded-md p-4">
    ...
    <div class="mt-3 grid md:grid-cols-2 lg:grid-cols-3 gap-3">
      <div v-for="t in state.gpuCatalog" :key="t.id" class="border rounded p-3">
        <div class="font-medium">{{ t.name }}</div>
        <div class="text-sm text-muted">{{ capacity(t) }}</div>
        <div class="mt-2 flex items-center gap-2 flex-wrap">
          <button class="px-2 py-1 rounded bg-surface border" @click="dec(t.id)">-</button>
          <input class="w-16 px-2 py-1 bg-bg border rounded" type="number" min="0" step="1" :value="count(t.id)" @input="onCount(t.id, $event)" />
          <button class="px-2 py-1 rounded bg-surface border" @click="inc(t.id)">+</button>
        </div>
      </div>
    </div>
